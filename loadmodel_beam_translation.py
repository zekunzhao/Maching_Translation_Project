...
def readLangs(lang1, lang2, reverse=False):
    ...
    lines = open('data_second/data/%s-%s.txt' % (lang1, lang2), encoding='utf-8').\
        read().strip().split('\n')
...
def train(input_tensor, target_tensor, encoder, decoder, encoder_optimizer, decoder_optimizer, criterion, max_length=MAX_LENGTH):
    ...
    if exact_decode:
        while True:
            ...
            print("\rworking...", nodes.qsize(),end="",flush=True)

            # print("before-nodes.get()")
            if nodes.qsize() == 0:
            ...
            score, n = nodes.get()
            f.write("\n--------searching--------- \n")
            f.write(str(n.currentList)+"score = "+str(score))

            decoder_input = n.wordid
            decoder_hidden = n.h
            # print("wordid :", decoder_input)
            ...
            topv, topi = decoder_output.topk(width)
            # print("width:",width)
            ...
            for new_k in range(width): 
                ...
                # if new_score < thresh_score:
                #     continue


                node = BeamSearchNode(decoder_hidden, n, currentList, decoded_t, new_score, n.leng + 1)
                diff = len(set(currentList)-set(target_tensor.squeeze().tolist()))+\
                        len(set(target_tensor.squeeze().tolist())-set(currentList))
                score = -(node.eval()+diff)
                nodes.put((score,node))

                f.write("\n--------golden-label---------\n")
                f.write(str(target_tensor.squeeze().tolist())+"score = "+str(golden_score.item()))
...
def evaluate(encoder, decoder, sentence, max_length=MAX_LENGTH):
    with torch.no_grad():
        ...
        #     if POPS[n.leng] >= 5  or n.leng > MAX_LENGTH:
        #         continue
        #     POPS[n.leng] += 1
        ...
        #     beam_width = len(decoder_output[0])
        #     # beam_width = 5
        #     topv, topi = decoder_output.topk(beam_width)

        #     for new_k in range(beam_width):
        #         decoded_t = topi[0][new_k].squeeze().detach()
        #         score = (topv[0][new_k]).item()
        #         currentList = n.currentList.copy()
        #         currentList.append(decoded_t.item())

        #         node = BeamSearchNode(decoder_hidden, n, currentList, decoded_t, n.score + score, n.leng + 1)
        #         score = -node.eval()
        #         nodes.put((score,node))
...
# trainIters(encoder1, attn_decoder1, 500, print_every=100, learning_rate=0.01)


# torch.save(encoder1.state_dict(),'data/model/encode1')
# torch.save(attn_decoder1.state_dict(),'data/model/attn_decoder1')

encoder1.load_state_dict(torch.load("data/model/encoder1.ppt"))
attn_decoder1.load_state_dict(torch.load('data/model/attn_decoder1.ppt'))
